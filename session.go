...
type session struct {
	...
	// readwriter is the in-memory representation for the copyist readwriter being read or
	// written by this session.
	readwriter *recordingSource
...
func newSession(source io.ReadWriteCloser, recordingName string) *session {
	return &session{
		recording:     recording{},
		readwriter:    newRecordingSource(source),
...
func (s *session) Close() {
	...
	if IsRecording() && len(s.recording) != 0 {
		...
		// the file.
		_ = s.readwriter.Parse()

		// Add the recording to the in-memory file and then write the file to
		// disk.
		s.readwriter.AddRecording(s.recordingName, s.recording)
		s.readwriter.WriteRecording()
	}
